...
const CategoryBreakDown = ({ isDark, tokens, data }: CategoryBreakDownProps) => {
  ...
  const chartDataWithColors = data.map((category) => {
    ...
    const percentage =
      totalMinutes > 0 ? Number(((category.total_minutes / totalMinutes) * 100).toFixed(1)) : 0;
  ...
  return (
    <View
      className={`${isDark ? 'border-border bg-card-bg' : 'border-border-light bg-card-bg-light'} overflow-hidden rounded-2xl py-3 pl-4`}
